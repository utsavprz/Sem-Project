...
def search(name="",price="",quantity="",supplier=""):
    supp_db = sqlite3.connect("products.db")
    cur = supp_db.cursor()
    cur.execute("SELECT * FROM ProductItem WHERE name=? OR price=? OR quantity=? OR supplier=?",(name, price, quantity, supplier))
    rows=cur.fetchall()
    supp_db.close()
    return rows
...
def update(id,name,price,quantity,supplier):
    supp_db = sqlite3.connect("products.db")
    cur = supp_db.cursor()
    cur.execute("UPDATE ProductItem SET name=?,price=?,quantity=?,supplier=? WHERE id=?", (name, price, quantity, supplier, id))
    supp_db.commit()
    supp_db.close()
